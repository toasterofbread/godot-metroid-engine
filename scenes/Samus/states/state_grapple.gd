...
func _init(_samus: KinematicBody2D):
	...
	self.animations = Animator.load_from_json("jump")
	self.collision_animations = Animator.load_from_json("neutral")
...
func process(_delta):
	...
	if collision and collision.normal.y < 0:
		collision_animations[animation].play(true)
		Samus.rotation = collision.normal.angle() + deg2rad(90)
	else:
		animations[animation].play(true)
		animations["legs"].play(true, false, true)
...
func set_aiming(angle: float):
	
	if angle == null:
		return
	
	angle = Global.invert_angle(rad2deg(angle))
...
func physics_process(delta: float):
	...
	Samus.rotation = (Vector2.UP.rotated(beam.global_position.angle_to_point(anchor.global_position)) * Vector2(1, 1)).angle()
