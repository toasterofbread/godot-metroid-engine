...
#		add_child(raycast)
#		raycast.cast_to = Vector2(0, -base_velocity/30)
...
func _physics_process(delta):
	...
	cast_to = Vector2(0, -base_velocity*0.1)
	...
	while is_colliding():
		...
		if collider.has_method("damage"):
			collider.damage(Weapon.damage_type, Weapon.damage_amount*(chargebeam_damage_multiplier if chargebeam_damage_multiplier != null else 1.0), get_node("ImpactPosition").global_position)
	clear_exceptions()
	
#	var colliding_bodies: Array = get_colliding_bodies()
#	for body in colliding_bodies:
#		if body.has_method("damage") and apply_damage:
